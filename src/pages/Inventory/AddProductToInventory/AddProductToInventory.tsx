...
const AddProductToInventory: React.FC = () => {
    ...
    const [allWarehouseProductData, setAllWarehouseProductData] = useState<Product[]>([]);
        ...
        (async () => {
            ...
            try {
                ...
                if (warehousesData?.length > 0) {
                    ...
                    let warehousesDataOptions = warehousesData.map((warehouse: Warehouse) => { return { id: warehouse.id, label: warehouse.name, value: warehouse.name } });
            ...
            if (warehouseId?.id) {
                try {
                    ...
                    if (productsData?.length > 0) {
                        ...
                        let productsDataOptions = productsData.map((product: Product) => { return { id: product.id, label: product.name, value: product.name } });
                ...
                try {
                    ...
                    if (warehouseBinsData?.length > 0) {
                        warehouseBinsData = warehouseBinsData.filter((warehouseBin: WarehouseBin) => (warehouseBin.maxUnits ?? 0) - (warehouseBin.currentStock ?? 0) > 0 );
                        let warehouseBinsDataOptions = warehouseBinsData.map((warehouseBin: WarehouseBin) => { return { id: warehouseBin.id, label: warehouseBin.binCode, value: warehouseBin.binCode, maxUnits: warehouseBin.maxUnits, currentStock: warehouseBin.currentStock } });
    ...
    useEffect(() => {
        (async () => {
            if (productId?.id) {
                ...
                try {
                    let variantsData = (allWarehouseProductData.flatMap((product: Product) => { return product.variants }));
                    if (variantsData?.length > 0) {
                        let variantsDataOptions = variantsData.map((variant: Variant) => { return { id: variant.id, label: variant.name, value: variant.name } });
    ...
    const handleAutoAllocate = async () => {
        ...
        try {
            let response = await new WarehouseBinService().allocateWarehouseBins(warehouse, product, Number(autoQty), variant)
            // Simulate API Call delay
            setTimeout(() => {
                const mockApiResult: BinTableRow[] = response?.allocations?.map((warehouseBin: WarehouseBin) => {
                    return {
                        id: warehouseBin.id, binCode: warehouseBin.binCode, maxQty: warehouseBin.maxUnits, availableQty: warehouseBin.currentStock, addQty: warehouseBin.currentStock?.toString()
                    }
                });
                //  [
            ...
            if (response.shortfall > 0) {
                setGlobalTableError("Unable To Fill The Total Quantity Free Up Some Rack");
                        ...
                        <div className="create-add-product-to-inventory-card">
                            <div className="create-add-product-to-inventory-grid-3">
                                <div className="create-add-product-to-inventory-field">
                                    ...
                                    <Dropdown noData={warehouseOptions?.length > 0 ? false : true} isLoading={isWarehouseLoading} label={warehouseId?.label ?? 'Select Warehouse'} options={warehouseOptions} selected={warehouseId} onSelect={(val: any) => setWarehouseId(val)} />
                                </div>
                                <div className="create-add-product-to-inventory-field">
                                    <label>Select Product <span className="req">*</span></label>
                                    <Dropdown noData={productOptions?.length > 0 ? false : true} isLoading={isProductLoading} label={productId?.label ?? 'Select Product'} options={productOptions} selected={productId} onSelect={(val: any) => setProductId(val)} />
                                </div>
                                <div className="create-add-product-to-inventory-field">
                                    <label>Select Variant <span className="req">*</span></label>
                                    <Dropdown noData={variantOptions?.length > 0 ? false : true} isLoading={isVariantLoading} label={variantId?.label ?? 'Select Variant'} options={variantOptions} selected={variantId} onSelect={(val: any) => setVariantId(val)} />
                                </div>
                            </div>
                        </div>
