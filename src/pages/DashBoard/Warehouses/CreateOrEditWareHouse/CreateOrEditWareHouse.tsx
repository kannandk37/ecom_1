...
const CreateOrEditWareHouse: React.FC = () => {
      ...
      if (isEditMode) {
        ...
        try {
          const warehouseResponse =
            await new WarehouseService().getWarehouseById(
              `/api/warehouses/${id}`,
            );
          ...
          const binResponse =
            await new WarehouseBinService().getWarehouseBinsByWareHouseId(
              `/api/warehousebins/${id}/warehouse`,
            );
  ...
  const buildAisleTree = (bins: any[]): BinAisle[] => {
    ...
    return Array.from(aisleMap.values()).sort((a, b) =>
      a.aisleCode.localeCompare(b.aisleCode),
    );
  };
  ...
  */
  ...
  const toggleAisleCollapse = (aisleId: string) => {
    setBinAisles((prev) =>
      prev.map((a) =>
        a.id === aisleId ? { ...a, collapsed: !a.collapsed } : a,
      ),
    );
  };
  ...
  const openAddRackModal = (aisleId: string) => {
    ...
    const existingLevels =
      aisleEntry?.racks[0]?.levels.length.toString() ?? "2";
  ...
  const confirmModal = async () => {
    ...
    try {
      if (modal.type === "aisle") {
        const nextAisleCode = nextCode(
          "A",
          binAisles.map((a) => a.aisleCode),
        );
        ...
        const nextRackCode = nextCode(
          "R",
          aisleEntry.racks.map((r) => r.rackCode),
        );
  ...
  const isValid = () => {
    ...
    if (!addressLine1) {
      setAddressLine1Error("This is required");
      return false;
    }
    if (!city) {
  ...
  const handleSubmit = async () => {
    if (isValid()) {
      ...
      try {
        ...
        if (isEditMode) {
          await new WarehouseService().updateWarehouseAndEssentialsById(
            id,
            warehouse,
            warehouseBin,
          );
        } else {
          await new WarehouseService().createWarehouseAndEssentials(
            warehouse,
            warehouseBins,
          );
        }
  ...
  const onChangeAddressLine1 = (v: string) => {
    if (v?.length > 60) {
  ...
  const onChangeAddressLine2 = (v: string) => {
    if (v?.length > 60) {
  ...
  const onChangePincode = (v: string) => {
    if (v?.length > 6) {
      setPincodeError("Only 6 digits allowed");
    } else {
      setPincode(v);
      setPincodeError(null);
    }
  };
  const onChangeMobile = (v: string) => {
    if (v?.length > 10) {
      setMobileError("Only 10 digits allowed");
    } else {
      setMobile(v);
      setMobileError(null);
    }
  };
  const onChangeTotalCapacity = (v: string) => {
    if (!v) {
      setTotalCapacityError("Please provide total capacity");
    } else {
      setTotalCapacity(v);
      setTotalCapacityError(null);
    }
  };
  const onChangeAisle = (v: string) => {
    if (!v) {
      setAisleError("Please provide aisle count");
    } else {
      setAisle(v);
      setAisleError(null);
    }
  };
  const onChangeRack = (v: string) => {
    if (!v) {
      setRackError("Please provide rack count");
    } else {
      setRack(v);
      setRackError(null);
    }
  };
  const onChangeLevel = (v: string) => {
    if (!v) {
      setLevelError("Please provide level count");
    } else {
      setLevel(v);
      setLevelError(null);
    }
  };
  const onChangeMaxUnits = (v: string) => {
    if (!v) {
      setMaxUnitsError("Please provide max units");
    } else {
      setMaxUnits(v);
  ...
  const onChangeState = (v: { id: string; label: string; value: string }) => {
    if (!v) {
            ...
            <div className="create-warehouse-card-body">
              ...
              <div className="create-warehouse-row-split-dropdowns">
                ...
                <div className="create-warehouse-field-group">
                  ...
                  <Dropdown
                    options={typeOptions}
                    label={type?.label || "Select Ownership Type"}
                    selected={type}
          ...
          <div className="create-warehouse-card">
            <div className="create-warehouse-card-header">
              ...
              <h2>
                {isEditMode
                  ? "Bin Infrastructure"
                  : "Initial Bin Infrastructure"}
              </h2>
              <span className="bin-summary-badge">
                {binAisles.length} aisle{binAisles.length !== 1 ? "s" : ""} ·{" "}
                {calcTotalBins(binAisles)} bins
              </span>
            </div>

            <div className="bin-edit-container">
              <div className="bin-edit-note">
                Manage your storage hierarchy. You can add or remove aisles,
                racks, and levels manually.
              </div>
                  ...
                  <div key={aisleEntry.id} className="bin-aisle-block">
                    ...
                    <div className="bin-aisle-header">
                      <div className="bin-aisle-header-left">
                        <span className="bin-aisle-name">
                          {aisleEntry.aisleCode}
                        </span>
                        <span className="bin-aisle-meta">
                          {aisleEntry.racks.length} rack
                          {aisleEntry.racks.length !== 1 ? "s" : ""} ·{" "}
                          {binCount} bins
                        </span>
                      </div>
                      <div className="bin-aisle-header-right">
                        ...
                        <button
                          ...
                          style={{ color: "#dc3545" }}
                          onClick={() => handleRemoveAisle(aisleEntry.id)}
                        >
                          <FiX size={12} /> Remove aisle
                        </button>
                        <button
                          className="bin-action-btn"
                          onClick={() => openAddRackModal(aisleEntry.id)}
                        >
                          <FiPlus size={12} /> Add rack
                        </button>
                        <button
                          className="bin-collapse-btn"
                          onClick={() => toggleAisleCollapse(aisleEntry.id)}
                        >
                      ...
                      <>
                        ...
                        {aisleEntry.racks.map((rackEntry) => (
                          <div key={rackEntry.id} className="bin-rack-row">
                            <span className="bin-col-rack">
                              {rackEntry.rackCode}
                            </span>
                            <div className="bin-col-levels">
                              <div
                                className="bin-level-pills"
                                style={{
                                  display: "flex",
                                  flexWrap: "wrap",
                                  gap: "8px",
                            ...
                            <div
                              className="bin-col-action"
                              style={{ display: "flex", gap: "8px" }}
                            >
                              <button
                                className="bin-add-level-btn"
                                onClick={() =>
                                  openAddLevelModal(aisleEntry.id, rackEntry.id)
                                }
                              >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleRemoveRack(aisleEntry.id, rackEntry.id)
                                }
                              >
                                <FiX size={12} /> rack
                              </button>
                ...
                <div className="bin-modal-overlay" onClick={closeModal}>
                  ...
                  >
                    <div className="bin-modal-header">
                      ...
                      <button className="bin-modal-close" onClick={closeModal}>
                        <FiX />
                      </button>
                    </div>
                        ...
                        <div className="bin-modal-grid-2">
                          <div className="create-warehouse-field-group">
                            ...
                            <DashBoardInput
                              ...
                              error={!!modal.racksError}
                              errorMessage={modal.racksError}
                            />
                          </div>
                          <div className="create-warehouse-field-group">
                            <label>
                              Levels <span className="req">*</span>
                            </label>
                            <DashBoardInput
                              type="number"
                              placeholder="e.g. 2"
                              ...
                              error={!!modal.levelsError}
                              errorMessage={modal.levelsError}
                            />
                          </div>
                        </div>
                      )}
                      ...
                      */}
                      ...
                      {getModalBinPreview() > 0 && (
                        <div className="bin-modal-preview">
                          This will generate{" "}
                          <strong>
                            {getModalBinPreview()} new bin
                            {getModalBinPreview() !== 1 ? "s" : ""}
                          </strong>
                        </div>
                      )}
                    </div>
