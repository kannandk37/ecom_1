...
import StockInfoCard from "../../../../assets/ui/StockCard/StockInfoCard";
...
const ManageStock: React.FC = () => {
  ...
  const [actionType, setActionType] = useState<{
    ...
    id: null,
    label: InventoryAction.ADJUST,
    value: InventoryAction.ADJUST,
  });
    ...
    (async () => {
      if (warehouseId?.id) {
        ...
        try {
          ...
          let warehouseBinOptions = warehouseBinsData.map(
  ...
  useEffect(() => {
    (async () => {
      if (binId?.id && productId?.id) {
        ...
        try {
          let binStockData =
            await new BinStockService().binStocksByWarehouseBinAndProduct(
              binId.id,
              productId.id,
            );
          setBinStocks(binStockData);
          let options = binStockData.map((binStock: BinStock) => {
            return {
              id: binStock.id,
              label: binStock.batchNumber,
              value: binStock.batchNumber,
            };
          });
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      if (
        binStockChecker &&
        binStockChecker.qtyOnHand - Number(quantity) <= 0
      ) {
        setOpenModal(true);
  ...
  const selectedBinStock = binStocks.find(
    (el: BinStock) => el.id === binStockId?.id,
  );
  return (
    <>
      {isLoading ? (
        ...
        <>
                ...
                <div className="create-Inventory-stock-dropdown-grid">
                  ...
                  <StockInfoCard
                    ...
                    label={
                      <>
                        Current Stock in{" "}
                        <b style={{ color: "#1a1a1a", fontSize: "0.85rem" }}>
                          {binStocks?.length > 0 && binStocks[0]?.bin?.binCode
                            ? binStocks[0].bin.binCode
                            : "Selected Bin"}
                        </b>
                      </>
                    }
                  ...
                  <StockInfoCard
                    ...
                    label={
                      <>
                        Total Stock in{" "}
                        <b style={{ color: "#1a1a1a", fontSize: "0.85rem" }}>
                          {warehouseId.value || "Select Warehouse"}
                        </b>
                      </>
                    }
              ...
              <div className="create-Inventory-stock-card">
                ...
                <div className="create-Inventory-stock-form-col">
                  <div className="create-Inventory-stock-row-split">
                        ...
                        {actionType.value === InventoryAction.ADJUST && (
                          <label className="create-Inventory-stock-radio-label">
                            <input
                              ...
                              onChange={() =>
                                setAdjustmentType(AdjustmentType.ADD)
                              }
                            />
                            Add (+)
                          </label>
                    ...
                    {actionType.label != InventoryAction.WASTE &&
                      adjustmentType == AdjustmentType.ADD && (
                        <div className="create-Inventory-stock-field">
                          <label>
                            Expiry Date{" "}
                            <span className="create-Inventory-stock-req">
                              *
                            </span>
                          </label>
                          <DashBoardInput
                            ...
                            value={
                              expiryDate
                                ? expiryDate?.toFormat("yyyy-MM-dd")
                                : ""
                            }
                            onChange={(e: string) => {
                              setExpiryDate(DateTime.fromJSDate(new Date(e)));
                            }}
                          />
                          {errors.expiryDate && (
                            <span className="create-Inventory-stock-error">
          ...
          {openModal && (
            <SimpleModal
              title={
                "Bin Stock Getting Emptied ! Can We Delete The Bin Stock Record ?"
              }
              subtitle={
                "Bin Stock quantity is becoming zero (0), so can we free up binstock by deleting ?"
              }
